...
Write-Host '[1/7] Checking Docker engine'
& cmd.exe /d /c 'docker info >nul 2>&1'
if ($LASTEXITCODE -ne 0) {
    throw 'Docker Desktop is not running.'
}

Write-Host '[2/7] Starting isolated network, Kali, and victim'
Invoke-Compose up -d --build victim kali

Write-Host '[3/7] Starting packet capture on Kali'
...
Write-Host '[4/7] Generating authorized lab traffic: ICMP, HTTP, SYN scan'
...
Write-Host '[5/7] Running Snort 3 offline analysis'
...
if (-not $SkipElastic) {
    Write-Host '[6/7] Starting Elasticsearch, Logstash, and Kibana'
    Invoke-Compose up -d elasticsearch logstash kibana

    Write-Host '[7/7] Verifying Elasticsearch ingestion'
    ...
    Write-Host '[6/7] -SkipElastic: Elastic Stack startup skipped'
    Write-Host '[7/7] PCAP and Snort alert generation complete'
}
